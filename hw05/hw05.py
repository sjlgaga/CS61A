...
def make_withdraw(balance, password):
    ...
    error_time=0
    error_pass=[]
    def bank(amount,trial):
        nonlocal balance,error_time,error_pass
        if error_time==3:
            s="'"+error_pass[0]+"', '"+error_pass[1]+"', '"+error_pass[2]+"'"
            return "Too many incorrect attempts. Attempts: ["+s+"]"
        if trial==password:
            if balance<amount:
                return 'Insufficient funds'
            else:
