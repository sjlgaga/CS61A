...
def reverse_iter(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [lst[len(lst)-i-1] for i in range(0,len(lst))]
...
def add_chars(w1, w2):
    ...
    len1=len(w1)
    len2=len(w2)
    def func(idx_w1,idx_w2):
        if idx_w2 >=len2:
            return ''
        if (idx_w1>=len1):
            return w2[idx_w2]+func(idx_w1+1,idx_w2+1)
        if w1[idx_w1]==w2[idx_w2]:
            return func(idx_w1+1,idx_w2+1)
        else:
            return w2[idx_w2]+func(idx_w1,idx_w2+1)
    return func(0,0)
